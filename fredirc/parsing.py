...
functions that parse specific (parts of) irc messages and extract some information for further processing.
Parsing errors might be raised on invalid input, but an accepted input does not automatically mean
that it conforms to the irc message grammar (as described in RFC 2812).
"""
...
def parse(message):
    """
    TODO raise ParserError
    Parse a message and return a triple containing prefix, command and paramaters.
    Relevant part of the protocol's grammar:
       message    =  [ ":" prefix SPACE ] command [ params ] crlf
       command    =  1*letter / 3digit
       params     =  *14( SPACE middle ) [ SPACE ":" trailing ]
                  =/ 14( SPACE middle ) [ SPACE [ ":" ] trailing ]
    Prefix and parameters may be None. On the prefix and so called 'trailing' parameters
    (i.e. parameters with whitespaces) the leading colon as well as leading and trailing
    whitespaces are removed.
    :param message: Raw irc message (without the CRLF)
    :type message: str
    :return: 3-element tuple
    :rtype: two strings (prefix and command) and a tuple of strings (parameters)
    """
    prefix = None
    params = None
    # Prefix
    ...
    if len(tmp_split) == 2:
        ...
        params_trailing = [param.strip() for param in params_trailing if len(param) > 0]
...
def parse_message_target(msg_target):
    ...
    for msgto in targets_split:
        # TODO does not completely implements the specs (no channel ids and collon delimiters)
        if msgto.startswith('#') or msgto.startswith('+') or msgto.startswith('&'):
            mt = MessageTarget(channel=msgto)
        elif '@' in msgto:
            user_split = msgto.split('@', 1)
            if '!' in user_split[0]:
                nick_split = user_split[0].split('!', 1)
                mt = MessageTarget(nick=nick_split[0], user=nick_split[1], host=user_split[1])
            else:
                if '%' in user_split[0]:
                    host_split = user_split[0].split('%', 1)
                    mt = MessageTarget(user=host_split[0], host=host_split[1], server=user_split[1])
                else:
                    mt = MessageTarget(user=user_split[0], server=user_split[1])
        elif '%' in msgto:
            user_split = msgto.split('%', 1)
...
class MessageTarget(object):
    """
    Represents a target of an IRC message. Represents the non-terminal 'msgto' in IRC's grammer.
    """

    def __init__(self, channel=None, nick=None, user=None, host=None, server=None, target_mask=None):
